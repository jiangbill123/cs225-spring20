...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (index < subroot->elements.size()){
        if (subroot->elements[index].key == key){ 
            return subroot->elements[index].value; 
        }
    }
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (!subroot->elements.empty()){
        if ((index < subroot->elements.size()) && (subroot->elements[index] == pair)) {
             return;
        }
    }
    if (subroot->is_leaf == true){
        ...
        unsigned int children_size = subroot->children[index]->elements.size();
        if(children_size >= order){
