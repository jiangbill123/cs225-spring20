...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> NNhelper(KDTreeNode* Node, KDTreeNode* parent,
                        const Point<Dim>& query, int dimension) const;
    void clean(KDTreeNode* subroot);
    Point<Dim> select(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned k, int dimension);
    unsigned partition(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned pivotIndex, int dimension);
    KDTreeNode* construct(vector<Point<Dim>>& list, unsigned left, unsigned right, int dimension);
    void copy(KDTreeNode* this_tree, KDTreeNode* other);
};
